...
Raw_data_filtered = stroke_mortality %>% 
  filter(
    Year == 2018,
  ...
  group_by(HBName) %>% 
  select(-c(Year, AgeGroup))

```

```{r}

table_tidyverse = Raw_data_filtered %>%
  gt()
...
```{r}
table_tidyverse_v2 <- table_tidyverse %>% 
  summary_rows(
    columns = everything(),
...
```{r}
table_tidyverseV3 = table_tidyverse_v2 %>% 
  grand_summary_rows(
    columns = everything(),
